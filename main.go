	...
	if msgCategory == ADD_EXPENSE {
		...
		if detailType[1] == ADD_EXPENSE {
			d = DetailMessage{
				...
				Cost_Zero:       "Awww! \U00100083 if the number is less than 1 that mean there is no cost!!\n\nCancelled",
			}
			...
			d = DetailMessage{
				...
				Cost_Zero:       "Awww! \U0010009E if the cost is less than 1 that mean there is no income!!\n\nCancelled",
			}
			handleAskDetail(event, message, userID, roomID, groupID, data, msgType, d)
